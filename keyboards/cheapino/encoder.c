...
__attribute__((weak)) bool encoder_click_user(void) {
    return false;
}
__attribute__((weak)) bool encoder_turned_user(bool clockwise) {
    return false;
}
...
void fix_encoder_action(matrix_row_t current_matrix[]) {
    ...
    if (colA && colB) {
        ...
        if (colABPressed) {
            ...
            colABPressed = false;
            bool res = encoder_turned_user(true);
            if (!res) {
                turned(true);
            }
        }
        ...
        if (colABPressed) {
            ...
            colABPressed = false;
            bool res = encoder_turned_user(false);
            if (!res) {
                turned(false);
            }
        }
